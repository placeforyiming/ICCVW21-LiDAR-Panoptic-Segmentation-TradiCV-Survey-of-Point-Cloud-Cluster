...
num_instance_in_label=open(save_path_for_prediction+"num_label.txt", "w")
num_instance_in_prediction=open(save_path_for_prediction+"num_prediction.txt", "w")

save_path_for_prediction="./method_predictions/sequences/08/predictions/"
if not os.path.exists(save_path_for_prediction):
	print ("inference semantic first")

provided_label_path="./Dataset/semanticKITTI/data_odometry_labels/dataset/sequences/val/08/labels/"
...
for i in range(len(lidar_list)):
	...
	provided_label_file=provided_label_path+path_list[-1][:len(path_list[-1])-3]+"label"


	A.open_scan(lidar_list[i])

	print (lidar_list[i])
	provided_label=np.fromfile(provided_label_file,dtype=np.uint32)

	all_diff_labels=np.unique(provided_label)
	total_num_gt=0
	for each_label in all_diff_labels:
		semantic_each = each_label & 0xFFFF
		semantic_each=label_transfer_dict[semantic_each]
		if np.logical_and(semantic_each<9,semantic_each>0) and np.sum(provided_label==each_label)>args.minimum_points:
			total_num_gt+=1
	num_instance_in_label.write(str(total_num_gt))
	num_instance_in_label.write('\n')
	...
	print (true_lable)
	num_instance_in_prediction.write(str(true_lable))
	num_instance_in_prediction.write('\n')
	...
	#plt.imsave('./sss.png',np.asarray(color_rgb).astype(np.uint8))
	#time.sleep(1)
...
num_instance_in_label.close()
num_instance_in_prediction.close()
